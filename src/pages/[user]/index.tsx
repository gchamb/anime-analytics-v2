...
const fetcherMutation = async (url: string, { arg }: { arg: { bio?: string; image?: string } }) => {
	return fetch(url, {
		method: "PATCH",
...
export default function Profile() {
	...
	const {
		...
		isMutating,
		error: profileError,
	} = useSWRMutation(user !== undefined ? `/api/user/profile?username=${router.query.user}` : null, fetcherMutation);

	const canSaveChanges = validationError.bioError === "" && validationError.profilePicError === "";
	...
	const saveChanges = async () => {
		try {
			...
			if (!response?.ok) {
				const validRes = z.object({ error: z.string() }).safeParse(await response?.json());
