  ...
  try {
    ...
    switch (req.method) {
      case "GET":
        ...
        return res
          .status(200)
          .json({ list: specificList, pages: Math.ceil(listCount / LIST_MAX) });
      case "POST":
      ...
      case "PUT":
        ...
        if (parsedPutData.updating === undefined) {
          if (parsedPutData.listRequestType === "rate") {
            return res
              .status(400)
              .json({ error: "Updating field is undefined." });
          }
          ...
          await prisma.list.update({
            ...
            data: {
              ...
              month:
                parsedPutData.updating.ratedAt !== undefined
                  ? month
                  : listItem.month,
              year:
                parsedPutData.updating.ratedAt !== undefined
      ...
      case "DELETE":
        ...
        if (!parsedDeleteData.success) {
          return res
            .status(400)
            .json({ error: parsedDeleteData.error.message });
        }
