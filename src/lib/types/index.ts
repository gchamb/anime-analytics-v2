...
export const BasicListRequestSchema = z.object({
  id: z.string().cuid(),
  listRequestType: z.union([
    z.literal(listType[0]),
    z.literal(listType[1]),
    z.literal(listType[2]),
  ]),
});
export const AnimeListRequestSchema = z
  .object({
    malId: z.number().int(),
    animeName: z.string(),
...
export const PutListRequestSchema = z
  .object({
    updating: z
      .object({
        rate: z.number().int().min(0).max(5).optional(),
...
export const profileViewRequestSchema = z
  .object({
...
export const monthlyDataSchema = z.object<{
  [key in z.infer<typeof monthsSchema>]: z.ZodArray<typeof ListRowSchema>;
}>({
...
export const barTitlesSchema = z.union([
  z.literal("Animes Per Month"),
  z.literal("Animes Per Rating"),
  z.literal("Animes Per Episode Group"),
  z.literal("Animes Per Year Released"),
]);
export const circleTitlesSchema = z.union([
  z.literal("Most Watched Genres"),
  z.literal("Most Watched Studios"),
]);
export const analyticsSchema = z.object({
  bar: z.object({
    ...
    "Animes Per Year Released": z.object<{
      [key: string]: z.ZodArray<typeof ListRowSchema>;
    }>({}),
  }),
  circle: z.object({
    "Most Watched Genres": z.object<{
      [key in JikanAnimeGenres]: z.ZodArray<typeof ListRowSchema>;
    }>({
...
export function formulateAnimeListRequest(
  anime: JikanAnime,
  list: AnimeListRequest["listRequestType"]
): AnimeListRequest {
  return {
    malId: anime.mal_id,
