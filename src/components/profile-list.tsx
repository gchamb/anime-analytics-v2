...
export default function ProfileList({ username }: { username: string }) {
  ...
  const mutateList = async (
    ...
    if (type === "watch" || type === "plan") {
      ...
      if (
        ...
        (router.query.list === "watch" || router.query.list === "plan")
      ) {
    ...
    if (res !== undefined) {
      if (!res.ok) {
        ...
        if (type === "delete") {
          setSuccess(
            `You successfully deleted ${animeName} from ${router.query.list} list`
          );
  ...
  return (
    <>
      <Head>
        <title>{`${properCase(username)}'s ${properCase(
      ...
      <RateDialog
        ...
        onSubmit={(rate: 0 | 5 | 2 | 4 | 1 | 3, date: Date) => {
          mutateList(
            ...
            router.query.list === "rate" ? "update" : "rate",
            rate,
            date
          );
        }}
          ...
          (serverError.success && (
            <div className="h-4/5 grid place-items-center">
              <p className="text-center">
                {fetchError ? String(fetchError) : serverError.data.error}
              </p>
            </div>
          ))}
        {data !== undefined &&
