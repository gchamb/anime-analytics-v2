...
function transformData(
  ...
  title: string;
  data: { malId: number; imageUrl: string; id: string }[];
}[] {
  let returnData: {
    title: string;
    data: { malId: number; imageUrl: string; id: string }[];
  ...
  for (const key of keys) {
    ...
    let dataChart:
      | Analytics["bar"][keyof Analytics["bar"]]
      | Analytics["circle"][keyof Analytics["circle"]];
    ...
    const formattedData = Object.entries(dataChart).map(
      ([title, data], idx) => {
        return { title, data };
      }
    );
